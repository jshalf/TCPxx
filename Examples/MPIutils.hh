...
#include <mpi.h>
/*
Architecture-specific definitions which are common
to all files. 
*/
// Pad arrays by this amount to prevent cache-line collisions
// > 35 works for SP2 with PowerIII nodes
// > 13 works for T3E/900
#define ArrayPad 37
...
#ifdef SGI
// Kludge to prevent inclusion of broken mpi++.h header file
// on some SGI's
#define MPIPP_H
#endif
...
public:
  ...
  inline int numProcs() { return nprocs; }
  ...
  MPIcomm *cartCreate(int ndims=3){
    ...
    MPI_Dims_create(this->nprocs,ndims,pdims);
  ...
  int getRankFromCoords(int *thecoords){
    int rank;
    MPI_Cart_rank(comm,thecoords,&rank);
    return rank;
  }
  /*
